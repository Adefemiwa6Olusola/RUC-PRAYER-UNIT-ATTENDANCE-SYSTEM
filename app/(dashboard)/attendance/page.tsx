...
export default function AttendancePage() {
  ...
  // Fetch centres and student count
  useEffect(() => {
    const initData = async () => {
      try {
        ...
        if (studentsRes && studentsRes.ok) {
          ...
          const total = sData?.total || sData?.students?.length || 0;
  ...
  // Real-time debounced matric lookup
  useEffect(() => {
    ...
    const cleanSuffix = trimmed.replace(/[^0-9a-zA-Z]/g, '');

    const timer = setTimeout(() => {
      fetch(`/api/students/lookup?suffix=${encodeURIComponent(cleanSuffix)}`)
        .then(res => res.json())
        .then(students => {
    ...
    try {
      ...
      if (!targetStudent) {
        if (lookupState.status === 'valid' && lookupState.student) {
          ...
          const cleanSuffix = rawInput.replace(/[^0-9a-zA-Z]/g, '');
          const lookupRes = await fetch(`/api/students/lookup?suffix=${encodeURIComponent(cleanSuffix)}`);
      ...
      const isDuplicateLocal = recentRecords.some(r => r.id === targetStudent.id || r.matricNo === targetStudent.matricNo);
      ...
      if (isDuplicateLocal) {
        ...
        if (recordedSuccessfully) {
          ...
          setRecentRecords(prev => [
            { ...targetStudent, locationName: currentCentreName, time: timeFormatted, status: 'PRESENT' },
            ...prev
          ]);
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', marginBottom: '1.5rem' }}>
        <div style={{ backgroundColor: 'white', borderRadius: '0.75rem', padding: '1.25rem', textAlign: 'center', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0' }}>
          ...
          <div style={{ fontSize: '0.75rem', color: '#6b7280', fontWeight: 500 }}>Total Students</div>
        </div>
        <div style={{ backgroundColor: 'white', borderRadius: '0.75rem', padding: '1.25rem', textAlign: 'center', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0' }}>
          <div style={{ fontSize: '2.25rem', fontWeight: 800, color: '#16a34a', marginBottom: '0.25rem' }}>{stats.todaySignIns}</div>
          <div style={{ fontSize: '0.75rem', color: '#6b7280', fontWeight: 500 }}>Today&apos;s Sign-ins</div>
        </div>
        <div style={{ backgroundColor: 'white', borderRadius: '0.75rem', padding: '1.25rem', textAlign: 'center', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0' }}>
          <div style={{ fontSize: '2.25rem', fontWeight: 800, color: '#dc2626', marginBottom: '0.25rem' }}>{stats.attendanceRate}%</div>
          <div style={{ fontSize: '0.75rem', color: '#6b7280', fontWeight: 500 }}>Attendance Rate</div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '1.5rem', alignItems: 'start' }} className="sign-in-grid">
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '0.75rem', padding: '2rem', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
            <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: '#111827', margin: 0 }}>Student Sign In</h2>
            <button
              onClick={() => {
              ...
              style={{
                ...
                borderRadius: '0.5rem', padding: '0.4rem 0.75rem', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer',
              }}
            >
              + Register Student
            </button>
          ...
          <form onSubmit={markAttendance}>
            {message.text && (
              <div style={{
                padding: '0.75rem', borderRadius: '0.5rem', marginBottom: '1rem', fontSize: '0.85rem', fontWeight: 600, textAlign: 'center',
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <input
                ...
                style={{
                  ...
                  border: lookupState.status === 'valid' ? '2px solid #16a34a' : lookupState.status === 'invalid' ? '2px solid #dc2626' : '1px solid #d1d5db',
                  fontSize: '0.95rem', outline: 'none', fontFamily: 'monospace',
                  transition: 'border-color 0.2s',
                }}
              />

              {/* Real-Time Auto Lookup Feedback Box */}
              {lookupState.status === 'valid' && (
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                    ...
                    <span style={{ fontFamily: 'monospace', fontWeight: 500, color: '#166534', fontSize: '0.75rem' }}>({lookupState.student?.matricNo})</span>
                  </div>
                ...
                <div style={{
                  ...
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: '0.375rem', padding: '0.35rem 0.65rem', fontSize: '0.75rem',
            ...
            <button
              ...
              style={{
                ...
                gap: '0.5rem', marginBottom: '1.25rem', opacity: isSubmitting ? 0.7 : 1,
              }}
            >
              <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              {isSubmitting ? 'Recording...' : 'Sign In'}
            </button>
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '0.75rem', padding: '2rem', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', border: '1px solid #f0f0f0' }}>
          ...
          <div style={{ maxHeight: '400px', overflowY: 'auto' }}>
            {recentRecords.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '3rem 1rem', color: '#9ca3af', fontSize: '0.8rem' }}>
      ...
      {/* Manual Student Entry Modal (Simplified: ONLY Full Name & Matric No) */}
      {isAddModalOpen && (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div>
                  ...
                  <input
                    ...
                    style={{ width: '100%', padding: '0.7rem', borderRadius: '0.5rem', border: '1px solid #d1d5db', fontSize: '0.85rem', outline: 'none' }}
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    style={{ width: '100%', padding: '0.7rem', borderRadius: '0.5rem', border: '1px solid #d1d5db', fontSize: '0.85rem', outline: 'none', fontFamily: 'monospace' }}
                  />
                </div>
              </div>

              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem', marginTop: '1.5rem', paddingTop: '0.75rem', borderTop: '1px solid #f3f4f6' }}>
                <button type="button" onClick={() => setIsAddModalOpen(false)} style={{ padding: '0.55rem 1rem', borderRadius: '0.5rem', fontSize: '0.8rem', fontWeight: 600, border: '1px solid #d1d5db', backgroundColor: 'white', cursor: 'pointer' }}>Cancel</button>
                <button type="submit" disabled={isSavingStudent} style={{ padding: '0.55rem 1.25rem', borderRadius: '0.5rem', fontSize: '0.8rem', fontWeight: 700, backgroundColor: '#002f63', color: 'white', border: 'none', cursor: 'pointer', opacity: isSavingStudent ? 0.7 : 1 }}>
